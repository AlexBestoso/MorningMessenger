class FormSelectList : public CoreObject{
	private:
	float menuX = 0;
	float menuY = 0;
	float menuZ = 0;
	...
	FormButton getOptionById(int id){
		if(id < 0 || id > sectionButtonOptionCount){
			FormButton foo;
			return foo;
		}
		return sectionButtonOptions[id];
	}
	...
	int mouseClick(int button, int state, float x, float y){
			if(hovered){
				...
				if(maxOptionsVisible <= 0){
                        	        for(int i=0; i<sectionButtonOptionCount; i++){
                        	                int val = sectionButtonOptions[i].mouseClick(button, state, x, y);
						if(val != -1){
							clickedOption = i;
							return val;
						}
